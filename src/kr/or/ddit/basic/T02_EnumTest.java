...
public class T02_EnumTest {
	...
	// 데이터값을 정해 줄 경우에는 생성자를 만들들어서 괄호속의 값이 변수에 저장되도록 해야 한다.
	public enum Season {
	...
	public static void main(String[] args) {
		/*
			...
			2. ordinal() => 열거형 상수가 정의된 순서값을 반환한다. (기분적으로 0부터 시작)
			3. valueOf("열거형 상수이름"); => 지정된 열거형에서 '열거형상수이름'과 일치하는 열거형상수를 반환
		 */
		...
		System.out.println("get메서드 => " + ss.getStr());	// 직접 정의한 getStr()
		System.out.println("---------------------------------------------");

		// 열거형 이름.values() => 데이터를 배열로 가져온다.
		Season[] enumArr = Season.values(); // => Map에서도 사용함. 비슷하게 쓸 수 있음.
...
//		System.out.println(city == City.valueOf("대구"));
		
		// 서울 : 0	부산 : 1	대구 : 2	광주 : 3	대전 : 4
		System.out.println("대구 => " + city.compareTo(City.대구));	// compareTo는 Comparable의 enum이 구현
		System.out.println("서울 => " + city.compareTo(City.서울));	// 순서를 기준으로 상수의 차이를 비교
